...
\tikzset{step/.style={draw=Teal,rounded corners=2mm,fill=PaleTeal,minimum width=22mm,minimum height=9mm,font=\sffamily\bfseries\small,text=Navy}}
\node[step] (m) {MODEL};
\node[step,right=of m] (r) {REPLY};
\node[step,right=of r] (v) {VALUE};
\node[step,right=of v] (d) {DECIDE};
\node[step,below=of v] (u) {UPDATE};
\draw[->,thick,Teal] (m)--(r);
\draw[->,thick,Teal] (r)--(v);
\draw[->,thick,Teal] (v)--(d);
\draw[->,thick,Teal] (d) |- (u);
...
\begin{longtable}{p{.27\textwidth}p{.29\textwidth}p{.36\textwidth}}
\toprule
